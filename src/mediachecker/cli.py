...
def main():
    ...
    parser.add_argument('-r', '--recursive', action='store_true', default=False, help='search for media files recursively (default: False)')
    parser.add_argument('-l', '--write-log', action='store_true', default=False, help='save results to log file(s) alongside media (default: False)')
    parser.add_argument('-f', '--full',      action='store_true', default=False, help='check all tracks, not just first audio track (default: False)')
...
def find_files_nonrecursive(base_dir, globexpression):
    ...
    list_path = [i for i in os.listdir(base_dir) if os.path.isfile(os.path.join(base_dir, i))]
